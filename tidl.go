...
// GetTrack func
func (t *Tidal) GetTrack(id string) (Track, error) {
	var s Track

	err := t.get("tracks/"+id, &url.Values{}, &s)
...
// GetPlaylist func
func (t *Tidal) GetPlaylists() ([]Playlist, error) {
	var s struct {
		Items []Playlist
	}

	err := t.get(fmt.Sprintf("users/%s/playlists", t.UserID), &url.Values{
		"limit": {"500"},
	}, &s)
